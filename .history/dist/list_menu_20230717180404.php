...
if (isset($_GET['hapus'])) {
  ...
  $check = $conn->query("SELECT * FROM order_item WHERE id_menu = $idMenu");
  if ($check->num_rows > 0) {
      $error = "Menu tidak dapat dihapus karena sedang digunakan dalam pemesanaan. Harap hapus atau ubah menu terlebih dahulu.";
  } else {
    // Get the record to fetch the file path
    $result = $conn->query("SELECT * FROM menu WHERE id_menu = $idMenu");
    ...
    if ($hapus) {
      // Delete the file if the record is deleted
